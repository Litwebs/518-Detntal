...
import { Phone, Mail, MapPin, Clock, AlertTriangle, Accessibility } from "lucide-react";
...
export default function Contact() {
  ...
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", treatment: "", message: "", consent: false });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: "", email: "", phone: "", treatment: "", message: "", consent: false });
  };
          ...
          <div className="grid gap-8 lg:grid-cols-2">
              ...
              <div className="space-y-6">
                <Card className="border-none shadow-md">
                  <CardContent className="p-6 space-y-4">
                    <h3 className="font-serif text-xl font-semibold">Practice Details</h3>
                    <a href="tel:01706527127" className="flex items-center gap-3 text-foreground hover:text-secondary transition-colors">
                      <Phone className="h-5 w-5 text-secondary" /> 01706 527127
                    </a>
                    <a href="mailto:admin@518dental.co.uk" className="flex items-center gap-3 text-foreground hover:text-secondary transition-colors">
                      <Mail className="h-5 w-5 text-secondary" /> admin@518dental.co.uk
                    </a>
                ...
                <Card className="border-none shadow-md">
                  <CardContent className="p-6 space-y-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-sm mb-1">Dental Emergency</h4>
                        <p className="text-sm text-muted-foreground">If you are experiencing a dental emergency during practice hours, please call 01706 527127 immediately. Outside of hours, please call NHS 111 for advice.</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Accessibility className="h-5 w-5 text-secondary mt-0.5" />
                      <div>
                        <h4 className="font-semibold text-sm mb-1">Disabled Access</h4>
                        <p className="text-sm text-muted-foreground">Our practice has ground floor access suitable for wheelchair users and those with limited mobility.</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <AnimatedSection>
              <div className="space-y-6">
                <Card className="border-none shadow-md">
                  <CardContent className="p-6">
                    <h3 className="font-serif text-xl font-semibold mb-4">Send Us a Message</h3>
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div>
                        ...
                        <select value={formData.treatment} onChange={(e) => setFormData({ ...formData, treatment: e.target.value })} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                      ...
                      <div><Label>Message *</Label><Textarea rows={4} value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} required /></div>
